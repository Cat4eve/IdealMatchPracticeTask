...
def draw_box(img: np.ndarray, xyxy: Tuple[int, int, int, int], label: str) -> None:
    ...
    y_top = max(y1, th + 6)
    cv2.rectangle(img, (x1, y_top - th - 6), (x1 + tw + 6, y_top), (255, 200, 0), -1)
...
def process_video(
    ...
    out_video = out_dir / f"{stem}_mobilenetv3.mp4"
        ...
        while True:
            ...
            for i in range(len(boxes)):
                ...
                if cls_name == "__background__":
                    continue
                if allowed_classes is not None and cls_name.lower() not in allowed_classes:
                    continue
                ...
                label_text = f"{cls_name} {c:.2f}"
                draw_box(frame, (xi1, yi1, xi2, yi2), label_text)
...
def parse_args() -> argparse.Namespace:
    ap = argparse.ArgumentParser(description="MobileNetV3 (SSDLite) video object detection")
    ap.add_argument("--input", nargs="+", default=["data"],
                    help="Input video path(s) or a directory. Default: ./data")
    ap.add_argument("--out", default="data/output/mobilenetv3",
                    help="Output directory. Default: ./data/output/mobilenetv3")
...
def main() -> None:
    ...
    allowed_classes = {c.lower() for c in classes} if classes else None

    print(f"Using device: {device}")
    if allowed_classes:
        print(f"Filtering to {len(allowed_classes)} class names from classes.json")
