...
def process_video(
    ...
    out_video = out_dir / f"{stem}_resnet.mp4"
...
def parse_args() -> argparse.Namespace:
    ap = argparse.ArgumentParser(description="ResNet-50/101 + background subtraction video detection")
    ap.add_argument("--input", nargs="+", default=["data"],
                    help="Input video path(s) or a directory. Default: ./data")
    ap.add_argument("--out", default=None,
                    help="Output directory. Default: ./data/output/<model>")
    ap.add_argument("--model", choices=["resnet50", "resnet101"], default="resnet50",
                    help="Classification backbone to use")
...
def main() -> None:
    ...
    out_dir = Path(args.out) if args.out else (base_dir / "data" / "output" / model_tag)
    ...
    print("Note: ResNet is a classifier. This script uses motion boxes + classification.")
